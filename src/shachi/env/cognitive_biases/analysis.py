...
def load_decision_data(
    folder_path: str = DECISION_DATA_FOLDER, format: bool = True
) -> pd.DataFrame:
    """
...
def load_model_characteristics(
    ...
    if fill_na_scores:
        ...
        df_mean_abs_bias["Chatbot Arena Score"] = df_mean_abs_bias["Chatbot Arena Score"].fillna(
            mean_score
        )
    ...
    if not incl_random:
        df_mean_abs_bias = df_mean_abs_bias[df_mean_abs_bias["model"] != "Random*"].reset_index(
            drop=True
        )
...
def plot_bias_heatmap(
    ...
    if save_plot_dir:
        plt.savefig(
            os.path.join(save_plot_dir, "bias_heatmap.pdf"), format="pdf", bbox_inches="tight"
        )
...
def plot_scatter(
    df: pd.DataFrame, label: str, dot_size: int = 1, dot_alpha: float = 0.5, save_plot: bool = True
):
    """
    ...
    if save_plot:
        plt.savefig(
            os.path.join(PLOT_OUTPUT_FOLDER, "cluster_plot.pdf"), format="pdf", bbox_inches="tight"
        )
...
def plot_dendrogram(
    ...
    sch.dendrogram(
        linkage_matrix, labels=df[label].values, orientation="right", link_color_func=color_func
    )
    ...
    if save_plot:
        plt.savefig(
            os.path.join(PLOT_OUTPUT_FOLDER, "dendrogram.pdf"), format="pdf", bbox_inches="tight"
        )
...
def plot_bubble_plot(
    ...
    if save_plot:
        plt.savefig(
            os.path.join(PLOT_OUTPUT_FOLDER, "bubble_plot.pdf"), format="pdf", bbox_inches="tight"
        )
